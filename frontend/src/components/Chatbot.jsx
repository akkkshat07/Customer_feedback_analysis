...
const mdComponents = {
    ...
    h2:     ({ node, ...props }) => <h2 className="text-sm font-bold mt-3 mb-1.5 text-soft-accent" {...props} />,
    h3:     ({ node, ...props }) => <h3 className="text-sm font-semibold mt-2 mb-1 text-soft-fg" {...props} />,
    strong: ({ node, ...props }) => <strong className="font-semibold text-soft-fg" {...props} />,
    hr:     ({ node, ...props }) => <hr className="my-3 border-soft-muted/20" {...props} />,
    blockquote: ({ node, ...props }) => (
        <blockquote className="border-l-4 border-soft-accent/40 bg-soft-accent/5 px-4 py-2 my-2 rounded-r-lg text-soft-muted italic" {...props} />
    ),
    table: ({ node, ...props }) => (
    ...
    th: ({ node, ...props }) => (
        <th className="border border-soft-muted/20 px-3 py-2 bg-soft-accent/5 text-soft-accent font-semibold text-left" {...props} />
    ),
    td: ({ node, ...props }) => (
        <td className="border border-soft-muted/10 px-3 py-1.5 text-soft-fg" {...props} />
    ),
    code: ({ node, inline, ...props }) => inline
        ? <code className="bg-soft-bg shadow-inset-sm px-1.5 py-0.5 rounded text-xs font-mono text-soft-accent" {...props} />
        : <code className="block bg-soft-bg shadow-inset p-3 rounded-lg text-xs font-mono overflow-x-auto my-2" {...props} />,
};
...
const StreamingMessage = ({ content, onDone }) => {
    ...
    return (
        <div className="prose prose-sm max-w-none text-soft-fg">
...
const WELCOME_MSG = {
    role: 'bot',
    content: "## Hi, I'm Esme 👋\n\nI'm your AI Customer Intelligence Agent with live access to **5,287 customer reviews** across Blue Heaven and Nature's Essence.\n\nAsk me anything — sentiment trends, product analysis, complaint breakdowns, or top customer quotes.",
    streaming: false,
};
...
const Chatbot = () => {
    ...
    return (
        <>
            ...
            <div className="h-full w-full flex bg-soft-bg overflow-hidden relative">
                ...
                <aside className={`
                    ...
                    bg-soft-bg shadow-extruded flex flex-col
                `}>
                    <div className="flex items-center justify-between px-4 py-3 border-b border-soft-muted/10 flex-shrink-0">
                        <span className="text-xs font-bold uppercase tracking-widest text-soft-muted">Chat History</span>
                        <button onClick={handleNewChat} className="flex items-center gap-1 px-2 py-1 rounded-md btn-neumorphic btn-primary text-xs font-semibold transition-colors">
                            <Plus size={12} /> New
                        </button>
                    </div>
                    <div className="flex-1 overflow-y-auto py-2 space-y-0.5 px-2" style={{ scrollbarWidth: 'thin' }}>
                        {sessions.length === 0 ? (
                            <p className="text-xs text-soft-muted text-center mt-8 px-3 leading-relaxed">No chat history yet.<br />Start a conversation!</p>
                            ...
                            <div key={session.id} className="group relative">
                                <button
                                    ...
                                    className={`w-full text-left px-3 py-2.5 rounded-lg transition-all ${
                                        activeId === session.id
                                            ? 'shadow-inset-sm bg-soft-bg'
                                            : 'hover:shadow-inset-sm'
                                    }`}
                                >
                                    <div className="flex items-start gap-2 pr-5">
                                        <MessageSquare size={12} className={`mt-0.5 flex-shrink-0 ${activeId === session.id ? 'text-soft-accent' : 'text-soft-muted'}`} />
                                        <div className="min-w-0">
                                            <p className={`text-xs font-medium truncate leading-snug ${activeId === session.id ? 'text-soft-accent' : 'text-soft-fg'}`}>
                                                {session.title}
                                            </p>
                                            <p className="text-[10px] text-soft-muted mt-0.5">{formatTime(session.updatedAt)}</p>
                                        </div>
                                    </div>
                                </button>
                                <button
                                    onClick={(e) => handleDeleteSession(e, session.id)}
                                    className="absolute right-2 top-1/2 -translate-y-1/2 p-1 rounded opacity-0 group-hover:opacity-100 transition-opacity text-soft-muted hover:text-red-500"
                                    title="Delete"
                                >
                                    <Trash2 size={12} />
                                </button>
                ...
                {/* ── Main panel ──────────────────────────────────────────── */}
                <div className="flex flex-col flex-1 min-w-0 bg-soft-bg overflow-hidden">

                    {/* Header */}
                    <div className="bg-soft-accent px-4 py-3 text-white flex items-center gap-3 shadow-extruded flex-shrink-0">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-white/80">Customer Intelligence · 5,287 reviews</p>
                        </div>
                        {!sidebarOpen && (
                    ...
                    <div
                        ref={scrollRef}
                        onScroll={onScroll}
                        className="flex-1 overflow-y-auto px-5 py-5 space-y-5 bg-soft-bg"
                        style={{ scrollbarWidth: 'thin', scrollbarColor: '#c8cdd5 transparent' }}
                    >
                        {messages.map((m, i) => (
                                ...
                                <div className={`flex items-end gap-2 max-w-[88%] ${m.role === 'user' ? 'flex-row-reverse' : ''}`}>
                                    ...
                                    <div className={`w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 mb-0.5 ${
                                        m.role === 'user'
                                            ...
                                            : 'bg-soft-bg shadow-extruded-sm text-soft-accent'
                                    }`}>
                                    ...
                                    <div className={`px-4 py-3 rounded-2xl text-sm leading-relaxed ${
                                        m.role === 'user'
                                            ? 'bg-soft-accent text-white rounded-br-sm shadow-extruded-sm'
                                            : 'bg-soft-bg text-soft-fg rounded-bl-sm shadow-extruded'
                                    }`}>
                                        {m.role === 'user' ? (
                        ...
                        {isLoading && (
                            <div className="flex justify-start msg-enter">
                                <div className="flex items-end gap-2">
                                    <div className="w-6 h-6 rounded-full bg-soft-bg shadow-extruded-sm text-soft-accent flex items-center justify-center flex-shrink-0">
                                        <Bot size={12} />
                                    </div>
                                    <div className="bg-soft-bg shadow-extruded px-3.5 py-2.5 rounded-2xl rounded-bl-sm flex items-center gap-2">
                                        <span className="text-xs text-soft-muted mr-1">Analyzing data</span>
                                        <ThinkingDots />
                                    </div>
                                </div>
                            </div>
                    ...
                    {/* Quick prompts */}
                    {showQuickPrompts && (
                        <div className="px-5 pb-3 flex-shrink-0 bg-soft-bg">
                            <div className="grid grid-cols-2 gap-2">
                                {QUICK_PROMPTS.map((p, i) => (
                                    <button key={i} onClick={() => handleSend(p)}
                                        className="text-left text-xs px-3 py-2.5 rounded-xl bg-soft-bg shadow-extruded-sm text-soft-fg hover:shadow-inset-sm hover:text-soft-accent transition-all">
                                        {p}
                                    </button>
                                ))}
                            </div>
                    ...
                    {/* Input */}
                    <div className="px-5 py-4 bg-soft-bg border-t border-soft-muted/10 flex-shrink-0">
                        {attachedFile && (
                            <div className="flex items-center gap-2 px-3 py-1.5 mb-2 bg-soft-accent/10 rounded-lg text-xs text-soft-accent font-medium w-fit max-w-xs">
                                <Paperclip size={12} />
                                <span className="truncate">{attachedFile.name}</span>
                                <button onClick={() => setAttachedFile(null)} className="ml-1 hover:text-red-500 transition-colors"><X size={12} /></button>
                            </div>
                        )}
                        <div className="flex items-end gap-2 input-neumorphic px-3 py-2 focus-within:ring-2 focus-within:ring-soft-accent/20 transition-all">
                            <button type="button" onClick={() => fileInputRef.current?.click()} title="Attach CSV"
                                className="p-1.5 text-soft-muted hover:text-soft-accent transition-colors flex-shrink-0">
                                <Paperclip size={18} />
                            </button>
                            ...
                            <textarea
                                ...
                                className="flex-1 bg-transparent border-none focus:ring-0 text-sm text-soft-fg placeholder-soft-muted outline-none disabled:opacity-50 resize-none overflow-y-auto leading-relaxed py-1"
                                style={{ minHeight: '28px', maxHeight: '120px' }}
                            />

                            <button type="button" onClick={toggleVoice} title={isRecording ? 'Stop' : 'Voice input'}
                                className={`p-1.5 transition-all flex-shrink-0 rounded-lg ${isRecording ? 'text-red-500 bg-red-50 animate-pulse' : 'text-soft-muted hover:text-soft-accent'}`}>
                                {isRecording ? <MicOff size={18} /> : <Mic size={18} />}
                            </button>

                            <button onClick={() => handleSend()} disabled={(!input.trim() && !attachedFile) || isLoading}
                                className="p-1.5 rounded-lg flex-shrink-0 transition-all disabled:text-soft-muted disabled:cursor-not-allowed btn-neumorphic btn-primary disabled:bg-soft-bg disabled:shadow-inset-sm">
                                <Send size={18} />
                            </button>
                        </div>
                        <p className="text-[10px] text-center text-soft-muted mt-2">Esme AI queries live data · CSV upload &amp; voice supported</p>
                    </div>
                </div>
            </div>
        </>
